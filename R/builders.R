...
#'
#' @param d The matrix of size \code{(n_T + n_lags) x n_determ} of
#' deterministic terms.
#' @param n_lags The number of lags
#' @keywords internal
#' @noRd
#'
#' @return \item{DD}{A matrix of size \code{n_T x ((n_lags + 1) * n_determ)} where
...
#'
#' Builds the parameter matrix of dynamic coefficients for the companion form
#' representation.
#'
#' @param Pi Matrix of size \code{n_vars x (n_vars * n_lags)} containing the
...
#'
#' @return
#' \item{Pi_comp}{The companion form matrix of size
#' \code{(n_vars * n_lags) x (n_vars * n_lags)}}.
...
#'
#' @param z Matrix of size \code{(n_T + n_lags) x n_vars} of data
#' @param n_lags Number of lags
#' @keywords internal
#' @noRd
#'
#' @return
#' \item{Z}{A matrix of size \code{n_T x (n_vars * n_lags)}.}
...
#'
#' @return
#' \item{U}{The \code{U} matrix, of size
#' \code{((n_lags + 1) * n_vars * n_determ) x (n_vars * n_determ)}.}
...
#'
#' @return
#' \item{Y_tilde}{A matrix of size \code{n_T x n_vars}.}
#' @details Note that \code{z} does not contain missing values; at this point,
#' the missing values have been replaced by values drawn using the simulation
#' smoother.
...
#' @param Lambda The aggregation matrix (size \code{n_vars x (n_vars * n_lags)})
#' @param Number of variables
#' @param Number of lags
...
#' @details The element \code{M_Lambda[[t]]} corresponds to \eqn{M_t\Lambda}.
#' Currently, if element \code{i} of \code{Y[t, ]} is \code{NA}, then row
#' \code{i} of \code{M_Lambda[[t]]} is all \code{NA}.
...
#' @param aggregation A character vector of length \code{n_vars} with elements
#' being \code{'identity'}, \code{'average'} or \code{'triangular'} to indicate
#' the type of aggregation scheme to assume
#' @param Number of lags
#' @keywords internal
#' @noRd
#' @return
#' \item{Lambda}{An \code{n_vars x (n_vars * n_pseudolags)} matrix, where
#' \code{n_pseudolags} is \code{max(5, n_lags)} if any variable uses the
#' triangular aggregation scheme, \code{max(3, n_lags)} if any uses the
#' quarterly average.}
#' @details The choice \code{aggregation = "identity"} means that what is
#' observed is assumed to be exactly the underlying variable, whereas
#' \code{aggregation = "average"} uses the quarterly average of the monthly
#' underlying observations. Lastly, \code{aggregation = "triangular"} uses the
#' triangular specification used by Mariano and Murasawa (2010).
build_Lambda <- function(aggregation, n_lags) {
  ...
  for (i in 1:n_vars) {
    ...
    if (aggregation[i] == "triangular") {
      fill_vec <- c(1 / 3, 2 / 3, 1, 2 / 3, 1 / 3, rep(0, n_pseudolags - 5)) / 3
    }
