...
gibbs_sampler_schorf_qf <- function(Y, prior_Pi_AR1, lambda1, lambda2, lambda3, n_lags, n_fcst = NULL, n_reps,
  ...
  ols_results <- ols_initialization(z = Y, d = d, n_lags = n_lags, n_T = n_T, n_vars = n_vars, n_determ = 1)
  ...
  lnpY0 <- - n_vars * 0.5 * determinant(crossprod(X_dum), logarithm = TRUE)$modulus -
   ...
   Z_comp <- build_Z(z = Y, n_lags = n_lags)
