#' Posterior sampler for mfbvar
#'
#' Function that sets up the MCMC sampling routine for obtaining samples from
#' the posterior distribution
#'
#' @param x Prior object of class \code{mfbvar_prior}
...
#' @param ... additional arguments, currently only accepting lists \code{fixate}
#' and \code{init} with fixed values for certain parameters, or custom initial
#' values
#' @return A list with the sampled parameters stored in arrays, and prior and
#' retrieved settings
mfbvar_sampler <- function(x, minn = FALSE, ssng = FALSE, ss = FALSE, dl = FALSE,
  ...
  params_info <- get_params_info(
    minn = minn, ssng = ssng, ss = ss, dl = dl,
    csv = csv, fsv = fsv, iw = iw,
    diffuse = diffuse
  )
  list_to_variables(
    params_info, envir, "required_params",
  ...
  list_to_variables(x, envir, prior_params)

  # Retrieve some additional variables
  init_vars <- variable_initialization(
    Y = Y, freq = freq, freqs = freqs,
    n_lags = n_lags, Lambda_ = Lambda_,
    n_thin = n_thin
  )
  list_to_variables(init_vars, envir, retrieved_params)

  ##############################################################################
  ## PRIOR-SPECIFIC INITIALIZATIONS

  if (iw || csv) {
    priors <- create_prior_Pi(
      lambda1 = lambda1,
      lambda2 = NULL,
      lambda3 = lambda3,
    ...
    list_to_variables(
      priors, envir, "prior_Pi_mean", "prior_Pi_Omega",
  ...
  if (fsv || diffuse) {
    ...
    list_to_variables(priors, envir, "prior_Pi_mean", "prior_Pi_Omega")
    prior_params <- sort(c(
      prior_params,
  ...
  if (csv) {
    init_csv <- csv_initialization(
      prior_phi = prior_phi,
      prior_sigma2 = prior_sigma2
    )
    list_to_variables(
      init_csv, envir, "phi_invvar", "phi_meaninvvar",
  ...
  if (fsv) {
    ...
    list_to_variables(
      init_fsv, envir, "priorsigmaidi", "priorsigmafac",
  ...
  if (ss || ssng) {
    init_ss <- ss_initialization(
      d = d, d_fcst = d_fcst,
      n_T = n_T, n_lags = n_lags, n_fcst = n_fcst
    )
    list_to_variables(
      init_ss, envir, "d_fcst_lags", "D_mat", "dt",
      "d1", "n_determ"
    )
  }
  ...
  if (ssng) {
    ...
    list_to_variables(init_ssng, envir, "c0", "c1", "s")
  } else if (ss) {
  ...
  if (dl) {
    ...
    list_to_variables(init_dl, envir, "a", "slice", "gig")
  } else if (diffuse || fsv) {
  ...
  init_params <- parameter_initialization(
    Y = Y, n_vars = n_vars,
  ...
  init_fixate <- fixate_initialization(fixate, params)
  list_to_variables(init_fixate, envir, paste0("fixate_", params))
  ...
  if ((ss || ssng) && fsv) {
    mcmc_ssng_fsv(
      Y[-(1:n_lags), ], Pi, psi, phi_mu, lambda_mu, omega, Z, Z_fcst,
  ...
  if (minn && fsv) {
    mcmc_minn_fsv(
      Y[-(1:n_lags), ], Pi, Z, Z_fcst, mu, phi, sigma, f, facload, latent,
  ...
  if ((ss || ssng) && iw) {
    mcmc_ssng_iw(
      Y[-(1:n_lags), ], Pi, Sigma, psi, phi_mu, lambda_mu, omega, Z, Z_fcst, Lambda_, prior_Pi_Omega, inv_prior_Pi_Omega, Omega_Pi, prior_Pi_mean,
  ...
  if (minn && iw) {
    mcmc_minn_iw(
      Y[-(1:n_lags), ], Pi, Sigma, Z, Z_fcst, Lambda_, prior_Pi_Omega, inv_prior_Pi_Omega,
  ...
  if ((ss || ssng) && csv) {
    mcmc_ssng_csv(
      Y[-(1:n_lags), ], Pi, Sigma, psi, phi_mu, lambda_mu, omega, Z, Z_fcst, phi, sigma, latent, Lambda_, prior_Pi_Omega, inv_prior_Pi_Omega, Omega_Pi, prior_Pi_mean,
  ...
  if (minn && csv) {
    mcmc_minn_csv(
      Y[-(1:n_lags), ], Pi, Sigma, Z, Z_fcst, phi, sigma, latent, Lambda_, prior_Pi_Omega, inv_prior_Pi_Omega,
  ...
  if (minn && diffuse) {
    mcmc_minn_diffuse(
      Y[-(1:n_lags), ], Pi, Sigma, Z, Z_fcst, aux, global, local, slice, Lambda_, prior_Pi_Omega,
  ...
  if ((ss || ssng) && diffuse) {
    mcmc_ssng_diffuse(
      Y[-(1:n_lags), ], Pi, Sigma, psi, phi_mu, lambda_mu,
