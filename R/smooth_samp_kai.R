...
## me : T   by p
{
  ...
  if(is.null(lH0)) lH0=lH
  u3 = smoothing(mZ=mZ-mZZ,mX=mX-mX,lH=lH0,mF=mF,mB=mB,mQ=mQ,iT=iT,ip=ip,iq=iq,is=is,h0=h0-h0,P0=P0,X0=X0-X0)
  mU = mE + u3
  # from 0 to iT
...
smoothing <- function(mZ,mX,lH,mF,mB,mQ,iT,ip,iq,is,h0,P0,X0)
{
  ...
  aS = mH%*%P1%*%t(mH)
  IS[[1]] = chol2inv(chol(aS)) ## smoothing
  #IS[[1]] = gsl_chol_invert(aS) ## smoothing
  ...
  for(iter in 2:iT){
    ...
    aS = mH%*%P1%*%t(mH)
    IS[[iter]] = chol2inv(chol(aS)) ## smoothing
    #IS[[iter]] = gsl_chol_invert(aS) ## smoothing
