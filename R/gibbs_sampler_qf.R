...
gibbs_sampler_qf <- function(Y, d, d_fcst = NULL, prior_Pi_mean, prior_Pi_Omega, prior_S, prior_nu, prior_psi_mean, prior_psi_Omega,
  ...
  ols_results <- tryCatch(ols_initialization(z = Y, d = d, n_lags = n_lags, n_T = n_T, n_vars = n_vars, n_determ = n_determ),
  ...
  for (r in 2:(n_reps)) {
    ...
    Pi_Sigma <- posterior_Pi_Sigma(Z_r1 = Y, d = d, psi_r1 = psi[r-1, , drop = FALSE], prior_Pi_mean, prior_Pi_Omega, inv_prior_Pi_Omega, Omega_Pi, prior_S, prior_nu, check_roots, n_vars, n_lags, n_T)
    Pi[,,r]      <- Pi_Sigma$Pi_r
    ...
    psi[r, ] <- posterior_psi(Pi_r = Pi[,, r], Sigma_r = Sigma[,, r], Z_r1 = Y, prior_psi_mean, prior_psi_Omega, D_mat, n_vars, n_lags, n_determ)
    ...
    if (!is.null(n_fcst)) {
      ...
      Z_fcst[1:n_lags, , r] <- Y[(n_T - n_lags+1):n_T,, r] - d[(n_T - n_lags+1):n_T, ] %*% t(matrix(psi[r, ], nrow = n_vars))
      for (h in 1:n_fcst) {
