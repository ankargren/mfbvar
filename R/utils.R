  ...
  for (i in seq_along(params)) {
    ...
    assign(
      ...
      switch(params[i],
        ...
        latent = array(initval,
          dim = c(n_T_, n_sv, n_reps / n_thin),
...
ss_initialization <- function(d, d_fcst, n_T, n_lags, n_fcst) {
  ...
  return(list(
    d_fcst_lags = d_fcst_lags, D_mat = D_mat, dt = dt, d1 = d1,
    n_determ = n_determ
  ))
}
...
csv_initialization <- function(prior_phi, prior_sigma2) {
  ...
  return(list(
    phi_invvar = phi_invvar, phi_meaninvvar = phi_meaninvvar,
