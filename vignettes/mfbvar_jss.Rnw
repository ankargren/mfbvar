...
\author{Sebastian Ankargren\\Uppsala University
...
\Keywords{vector autoregression, steady-state prior, Minnesota prior, time series, \proglang{R}}
\Plainkeywords{vector autoregression, steady-state prior, Minnesota prior, stochastic volatility, time series, R}
...
\end{equation}
The VAR($p$) model can be written in companion form, where we let $z_t=(x_t', x_{t-1}', \dots, x_{t-p+1}')'$. Thus, we obtain
\begin{equation}
...
X=W \Gamma + E,
\end{align}
where $W=(W_1, \dots, W_T)'$ with $W_t=(x_{t-1}', \dots, x_{t-p}', 1)'$, $E=(\epsilon_1, \dots, \epsilon_T)'$, and $\Gamma=(\Phi', \phi)'$. The Minnesota prior for $\Gamma$ takes one of two forms depending on the specification of the error covariance component. We will use the term ``Minnesota prior'' to refer to the prior for the model with intercept in order to more easily contrast it with an alternative specification discussed in a later subsection. It does not, however, refer to the original Minnesota prior with a fixed diagonal error covariance matrix as used by \cite{Litterman1986}; rather, it should be interpreted as a normal prior for the regression parameters---including intercept---based on the Minnesota prior beliefs.
...
\begin{align}
\underline{\Gamma}(\underline{\gamma}) &= \begin{pmatrix}\operatorname{diag}(\underline{\gamma}) &  0_{n\times[(p-1)+1]}\end{pmatrix}'\label{eq:gamma}\\
...
\begin{align}
\Sigma_t=\VAR(\epsilon_t|\Lambda^f, \Omega_t^\nu, \Omega_t^f)=\Lambda^f\Omega_t^f\Lambda^{f'}+\Omega_t^\nu.
\end{align}
...
\begin{align}
y_{\mathcal{O}_{t-1},t-1:t-p}=(y_{\mathcal{O}_{t-1},t-1}', \dots, y_{\mathcal{O}_{t-1},t-p}')'
\end{align}
...
=\frac{1}{\kappa}\frac{|\Sigma_x|^{-1/2}}{2\pi^{n_x/2}}\exp\left\{-\frac{1}{2}(x-\hat{\mu}_x)'\Sigma_x^{-1}(x-\hat{\mu}_x)\right\}\\
\times{I}\left\{(x-\hat{\mu}_x)'\Sigma_x^{-1}(x-\hat{\mu}_x) \leq \chi^2_{n_x}(\kappa)\right\}\label{eq:f},
\end{multline}
...
\hat{\Sigma}_{Z}&=\frac{1}{R}\sum_{r=1}^R Z^{(r)}Z^{(r)'}-\hat{\mu}_{Z}\hat{\mu}_{Z}'.
\end{align}
The denominator term in \eqref{eq:mddminn} is the density of a matrix $t$ distribution with parameters $\operatorname{Mt}(W^{(r)}\underline{\Gamma},(I_T+ W^{(r)} \underline{\Xi}W^{(r)'})^{-1},\underline{S},\underline{\nu})$ (see \citealp{Karlsson2013} for the density and more details).
...
\begin{itemize}
\item \code{Y}: a matrix, data frame or tibble with the data. Quarterly variables should be in the final columns of \code{Y} and store quarterly observations in the final month of each quarter (and \code{NA} in the remaining cells). For full compatibility with plotting and post-processing functions, the column names should be meaningful variable names. Row names should be monthly dates formatted as \code{YYYY-MM-DD}. If \code{Y} is a data frame, a single non-numeric column is allowed to be included, which is then assumed to include the dates. The \pkg{mfbvar} package provides some support for single-frequency models in which no observations are missing; in this case, \code{freq} must be specified accordingly.
...
\item \code{freq}: a character vector indicating the sampling frequency. Either \code{"m"} for monthly variables or \code{"q"} for quarterly variables.
...
&\code{freq} & frequencies of variables & &\checkmark&\checkmark & \checkmark&\checkmark&\checkmark & \checkmark&\checkmark \\
...
&\code{n_burnin} & burn-in draws & & \checkmark&\checkmark& \checkmark&\checkmark&\checkmark & \checkmark&\checkmark\\
...
\item \code{update\_csv}, \code{update\_fsv}, \code{update\_ssng}: functions for the sampling steps required for common stochastic volatility, factor stochastic volatility, and the hierarchical steady-state prior
\end{itemize}
...
The data start in January 1980 and we retrieve the vintage available on December 10, 2018. The \pkg{mfbvar} package expects data to be provided in a data frame or a matrix. Each row represents a month, and quarterly variables should store observations on the final month of each quarter and \code{NA} elsewhere. The data obtained from FRED needs two adjustments before being used by \pkg{mfbvar}. First, quarterly GDP is stored on the first month of each quarter and needs to be moved to the end of the quarter for compatibility with \pkg{mfbvar}. The observed date of GDP is therefore shifted by two months.
<<>>=
out[[3]]$date <- out[[3]]$date + months(2)
@
Second, consumer price index and real GDP need to be transformed into annualized growth rates.
<<>>=
...
<<>>=
if (load_data) {
  load("mod_ss_iw.RData")
  load("mod_ssng_iw.RData")
} else {
  mod_ss_iw  <- estimate_mfbvar(prior, prior = "ss", variance = "iw")
  mod_ssng_iw <- estimate_mfbvar(prior, prior = "ssng", variance = "iw")
  if (store_data) {
    save(mod_ss_iw, file = "vignettes/mod_ss_iw.RData")
    save(mod_ssng_iw, file = "vignettes/mod_ssng_iw.RData")
  }
}
@
...
<<>>=
if (load_data) {
  load("mod_ss_csv.RData")
  load("mod_ss_fsv.RData")
} else {
  mod_ss_csv  <- estimate_mfbvar(prior, prior = "ss", variance = "csv")
  mod_ss_fsv <- estimate_mfbvar(prior, prior = "ss", variance = "fsv",
                                n_fac = 1)
  if (store_data) {
    save(mod_ss_csv, file = "vignettes/mod_ss_csv.RData")
    save(mod_ss_fsv, file = "vignettes/mod_ss_fsv.RData")
  }
}
@
